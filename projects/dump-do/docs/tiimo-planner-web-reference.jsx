...
const createDefaultTasks = () => [
  { id: 1, title: 'Morning Stretch', icon: 'dumbbell', colorName: 'mint', time: '07:00', duration: 15, completed: {}, date: todayString, recurrence: { type: 'daily', endDate: null, days: [] }, reminder: '5min' },
  { id: 2, title: 'Breakfast', icon: 'coffee', colorName: 'sunshine', time: '07:30', duration: 30, completed: {}, date: todayString, recurrence: { type: 'none' }, reminder: 'none' },
  { id: 3, title: 'Deep Work', icon: 'briefcase', colorName: 'sky', time: '09:00', duration: 90, completed: {}, date: todayString, recurrence: { type: 'weekdays', endDate: null, days: [] }, reminder: '15min' },
  { id: 4, title: 'Team Standup', icon: 'heart', colorName: 'rose', time: '10:00', duration: 30, completed: {}, date: todayString, recurrence: { type: 'weekly', endDate: null, days: [1, 3, 5] }, reminder: '5min' },
  { id: 5, title: 'Lunch Break', icon: 'utensils', colorName: 'coral', time: '12:00', duration: 45, completed: {}, date: todayString, recurrence: { type: 'none' }, reminder: 'atTime' },
  { id: 6, title: 'Reading', icon: 'book', colorName: 'lavender', time: '20:00', duration: 30, completed: {}, date: todayString, recurrence: { type: 'daily', endDate: null, days: [] }, reminder: '10min' },
];

const createDefaultRoutines = () => [
  { id: 'morning', name: 'Morning Routine', icon: 'sunrise', colorName: 'sunshine', isDefault: true, tasks: [
    { title: 'Wake Up & Hydrate', icon: 'sun', colorName: 'sunshine', time: '06:30', duration: 10, reminder: 'none' },
    { title: 'Morning Stretch', icon: 'dumbbell', colorName: 'mint', time: '06:45', duration: 15, reminder: '5min' },
    { title: 'Healthy Breakfast', icon: 'coffee', colorName: 'coral', time: '07:30', duration: 30, reminder: 'atTime' },
  ]},
  { id: 'work', name: 'Work Day', icon: 'laptop', colorName: 'sky', isDefault: true, tasks: [
    { title: 'Check Emails', icon: 'briefcase', colorName: 'sky', time: '09:00', duration: 30, reminder: 'atTime' },
    { title: 'Deep Work Block', icon: 'sparkles', colorName: 'lavender', time: '09:30', duration: 90, reminder: '5min' },
    { title: 'Lunch', icon: 'utensils', colorName: 'coral', time: '12:00', duration: 60, reminder: 'atTime' },
  ]},
  { id: 'evening', name: 'Evening Wind Down', icon: 'sunset', colorName: 'rose', isDefault: true, tasks: [
    { title: 'Evening Walk', icon: 'heart', colorName: 'mint', time: '18:00', duration: 30, reminder: '10min' },
    { title: 'Dinner', icon: 'utensils', colorName: 'coral', time: '18:30', duration: 45, reminder: 'atTime' },
    { title: 'Reading', icon: 'book', colorName: 'periwinkle', time: '20:30', duration: 30, reminder: '15min' },
  ]},
];
...
function TiimoPlanner() {
  ...
  const getTasksForDate = useCallback((date) => {
    ...
    return tasks
      .filter(t => shouldTaskAppearOnDate(t, dateStr))
  ...
  const copySingleTask = (task, targetDates) => {
    const newTasks = targetDates.map((date, index) => ({
      ...task, id: Date.now() + index, date: getDateString(date), completed: {}, recurrence: { type: 'none' },
    }));
    delete newTasks.color; // Remove runtime color, keep colorName
    setTasks(prev => [...prev, ...newTasks.map(t => ({ ...t, color: undefined }))]);
  ...
  return (
    <div className={`min-h-screen ${colors.bg} touch-pan-y transition-colors duration-300`}>
          ...
          {viewMode === 'day' && (
            <div className="flex gap-1.5 mt-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
              {weekDays.map((day) => {
                ...
                const isToday = getDateString(day) === todayString;
      ...
      {showSettings && <SettingsModal onClose={() => setShowSettings(false)} onDataCleared={() => { setTasks(createDefaultTasks()); setRoutines(createDefaultRoutines()); }} />}
...
function SettingsModal({ onClose, onDataCleared }) {
      ...
      <div className={`absolute bottom-0 left-0 right-0 ${colors.modal} rounded-t-3xl p-6 pb-8 safe-area-bottom animate-slide-up max-h-[85vh] overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div className={`p-4 rounded-2xl ${isDark ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
            ...
            <button onClick={() => setShowClearConfirm(true)} className={`w-full flex items-center justify-center gap-2 p-4 rounded-xl mt-3 ${isDark ? 'bg-red-900/30 text-red-400' : 'bg-red-50 text-red-600'}`}>
...
function AddTaskModal({ onClose, onSave, selectedDate, editingTask }) {
  ...
  const handleSave = () => {
    ...
    onSave({ title, icon, colorName, time, duration, recurrence, reminder, completed: editingTask?.completed || {} });
  };
